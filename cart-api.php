<?php
// Fixed cart-api.php with proper stock management and corrected response handling
...
switch ($action) {
    case 'add':
        ...
        $existing_cart = mysqli_query($conn, "SELECT * FROM cart WHERE customer_id = $customer_id AND product_id = $product_id");
        ...
        if (mysqli_num_rows($existing_cart) > 0) {
            ...
            $insert_query = "INSERT INTO cart (customer_id, product_id, quantity, added_at) VALUES ($customer_id, $product_id, $quantity, NOW())";
    ...
    case 'update':
        ...
        $current_query = mysqli_query($conn, "SELECT quantity FROM cart WHERE customer_id = $customer_id AND product_id = $product_id");
